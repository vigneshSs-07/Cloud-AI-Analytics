...
class ParseAndGetEventTimestamp(beam.PTransform):
    def expand(self, pcoll):
        return (
            pcoll
            | 'ParseJson' >> beam.Map(parse_json)# TODO: Apply parse_json function
            | 'GetEventTimestamp' >> beam.ParDo(GetEventTimestampFn()) # TODO: Apply GetEventTimestampFn DoFn.
            )
...
FROM
    TUMBLE(
        ...
        DESCRIPTOR(ts),
        'INTERVAL 1 MINUTE')
GROUP BY window_start""" # TODO: Write SQL Query

    # Create the pipeline
    p = beam.Pipeline(options=options)

    (p | 'ReadFromPubSub' >> beam.io.ReadFromPubSub(input_topic) # TODO: Read from Pub/Sub Topic
       | 'ParseAndGetEventTimestamp' >> ParseAndGetEventTimestamp().with_output_types(CommonLog) # TODO: Apply ParseAndGetEventTimestamp custom PTransform with output type CommonLog
